...
(define (domain freecell)
  (:requirements :strips) 
  (:predicates (on ?c1 ?c2) ;c1 is on c2
	       (incell ?c) ;card is in a freecell
	       (clear ?c) ;nothing stacked on c (on top of a column)
	       (cellspace ?n) ;there are n free freecells
	       (colspace ?n) ;there are n free columns
	       (home ?c) ;card is a top card in a home stack
	       (bottomcol ?c) ;card is at bottom of a stack
	       (canstack ?c1 ?c2) ;can stack c1 on c2
	       (suit ?c ?s) ;if card c is of suit s
	       (value ?c ?v) ;if card c is of value v
	       (successor ?n1 ?n0) ;if n1 = n0+1
 )
...
;;; not we generate a new stack.
  ...
  (:action move
	   :parameters (?card ?oldcard ?newcard) ;move a card stacked on oldcard to stack on newcard
  ...
  (:action move-b ;moving the only card of a column to another place -> 1 more free col
